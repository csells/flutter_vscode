# Reference

Reference material for APIs, conventions, and generated outputs.

## Contents

- [Generated Host API](generated-host-api.md) — the one generated Dart
  Extension Host API layer and its coverage workflow
- [Flutter View Protocol](view-protocol.md) — the version-2 Host Dart to
  Flutter View wire contract behind `FlutterViewHost` and `ViewShell`
- [Flutter View Startup](startup.md) — measured cold start from webview
  load to first rendered frame
- [VS Code API Parity](parity.md) — generated burn-down of emitted,
  excluded, and pending public API surface
- Annotation and generation conventions (see [Code Generation Rules](../agent-guidelines/code-generation.md))
- Package and build conventions (see [Package Conventions](../agent-guidelines/package-conventions.md))

## Related

- [Documentation Index](../index.md)
- [Agent Guidelines](../agent-guidelines/index.md)
