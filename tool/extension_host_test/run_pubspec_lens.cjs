...
const extensionProject = process.env.FLUTTER_VSCODE_PUBSPEC_LENS_PROJECT;
if (!extensionProject) {
  throw new Error('FLUTTER_VSCODE_PUBSPEC_LENS_PROJECT is required');
}
const workspacePath = process.env.FLUTTER_VSCODE_PUBSPEC_LENS_WORKSPACE;
if (!workspacePath) {
  throw new Error('FLUTTER_VSCODE_PUBSPEC_LENS_WORKSPACE is required');
}
...
async function main() {
  ...
  if (!fs.existsSync(path.join(workspacePath, 'pubspec.yaml'))) {
    throw new Error(`Workspace has no pubspec.yaml: ${workspacePath}`);
  }

  const profileRoot = fs.mkdtempSync(
    path.join(os.tmpdir(), 'flutter-vscode-pubspec-lens-host-'),
  );
  ...
  try {
    console.log(
      `[pubspec-lens-test] resolving VS Code test runtime ${vscodeVersion}`,
    );
    ...
    console.log(`[pubspec-lens-test] installing ${vsixPath}`);
    installVsix(vscodeExecutablePath, extensionsDir, userDataDir);

    console.log('[pubspec-lens-test] launching the pubspec workspace');
    await runTests({
...
main().catch((error) => {
  console.error('[pubspec-lens-test] failed');
  console.error(error);
  process.exitCode = 1;
});
